const quiz = {
  ...
  questions: [
    ...
    {
      question:
        "Who plays 'Will', an old friend of Monica and Ross from High School?",
      choices: ["Bruce Willis", "Paul Rudd", "Brad Pitt", "Michael Rapaport"],
      answerIndex: 2,
    },
    {
      question:
        "What does Monica's dad give her to compensate for ruining her childhood possessions?",
  ...
  $(".display").html(`
    <section class="display-view">
        <p class="display-text">${
          quiz.questions[playerStats.question].question
        }</p>
    </section>
...
function displayUserStats() {
  ...
  $(".progress").text(
    `Question: ${playerStats.question + 1} of ${numberOfQuestions}`
  );
  $(".score").text(
    `Score: (${playerStats.correct} correct, ${playerStats.incorrect} incorrect)`
  );
}
...
function displayFinalStatsView() {
  ...
  $(".display").html(`
        <section class="final-view">
            ...
            <p class="display-score">Score: ${
              (playerStats.correct / quiz.questions.length) * 100
            }%</p>
            <p class="display-totalCorrect">Total Correct: ${
              playerStats.correct
            }</p>
            <p class="display-totalIncorrect">Total Inccorrect: ${
